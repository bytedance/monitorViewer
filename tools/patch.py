...
sys.path.insert(0, str(os.environ['MONITOR_VIEWER_INSTALL_PATH']) + '/common')
...
def read_args():
    ...
    if not os.path.exists(args.patch_path):
        common_monitor.bprint('"' + str(args.path_path) + '": No such patch path.', level='Error')
        sys.exit(1)
...
class Patch():
    def __init__(self, patch_path):
        self.install_path = os.path.realpath(os.environ['MONITOR_VIEWER_INSTALL_PATH'])
    ...
    def check_path_name(self):
        ...
        if os.path.basename(self.install_path) != os.path.basename(self.patch_path):
            common_monitor.bprint('Current install path name is "' + str(os.path.basename(self.install_path)) + '", but patch path name is "' + str(os.path.basename(self.patch_path)) + '".', level='Warning')
                ...
                if (py_file not in install_py_list) or (not filecmp.cmp(abs_install_py, abs_patch_py)):
                    ...
                    if py_file in install_py_list:
                        ...
                        try:
                            ...
                            common_monitor.bprint('Failed on removing old python file "' + str(abs_install_py) + '".', level='Error')
                            common_monitor.bprint(error, color='red', display_method=1, indent=9)
                            sys.exit(1)
                    ...
                    if not os.path.exists(abs_install_path):
                        ...
                        try:
                            ...
                            common_monitor.bprint('Failed on creating directory "' + str(abs_install_path) + '".', level='Error')
                            common_monitor.bprint(error, color='red', display_method=1, indent=9)
                            sys.exit(1)
                    ...
                    try:
                        ...
                        common_monitor.bprint('Failed on copying file "' + str(abs_patch_py) + '" into "' + str(abs_install_py) + '".', level='Error')
                        common_monitor.bprint(error, color='red', display_method=1, indent=9)
                        sys.exit(1)
